...
router.post('/register', photoUpload.single('photo'), async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      token,
      user: { _id: user._id, name: user.name, email: user.email, role: user.role, photo: user.photo, phone: user.phone }
    });
  } catch (error) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({
      ...
      user: { _id: user._id, name: user.name, email: user.email, role: user.role, photo: user.photo, phone: user.phone, skills: user.skills }
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
// One-time admin setup — GET route
router.get('/setup-admin', async (req, res) => {
  try {
    const existing = await User.findOne({ email: 'admin@company.com' });
    if (existing) return res.json({ message: 'Admin already exists! Login with admin@company.com / admin123' });
    const hash = await bcrypt.hash('admin123', 10);
    await User.create({ name: 'HR Admin', email: 'admin@company.com', password: hash, role: 'admin', photo: null });
    res.json({ message: 'Admin created! Login with admin@company.com / admin123' });
  } catch (e) {
